...
Our target variable is price, a right skewed variable with a long right tail. On average in our sample apartment costed almost twice as much as rooms, which is perfectly understandable given that the freedom of a whole apartment is something that people are willing to pay more. In total we have a bit more than 11k observations in which the number of apartments is almost four times more than the number of rooms. 
...
We set the tuning parameters and stopping rules for each model. In case of CART, we used a stopping rule that limits the level to be nine. At the random forest, the algorithm picked the best model among 9 different models. On one hand, we defined the number of variables to consider for each split when growing the decorrelated trees: 8, 10 and 12 variables were set. We also defined the stopping rule in terms of the minimum number of observations required for the terminal nodes: 5, 10 and 15 observations. Thus, we had 3*3 different random forest model and we picked the best one for the further analysis. For GBM, we applied the next parameters: complexity of trees; number of trees; learning rate; and the minimum number of training set samples in a node to commence.
...
After running the models, we compared them based on their performance to pick the best one for the price prediction. We ran our models on the test set (coming from cross-validation slicing) and also, we ran them on the holdout set for the model comparison. The Root Mean Squared Error (RMSE) serves to measure the prediction error, so we picked the model having the least RMSE value. In our case, it was the random forest model, where RMSE was 49.742 in the test set and 49.910 in the holdout set. Both of them was considered as the lowest RMSE.
...
The TOP10 most important variables are shown on the plot below. The most important variable (~6.6%) in the random forest model for pricing is number of reviews per month, and the forth most important is the number of reviews. The number of accommodates is on the second most important variable in pricing. As a benchmark, we can use the "Predicting Airbnb Prices in London with Random Forest" case study (Békés, G., & Kézdi, G. (2021). Data Analysis for Business, Economics, and Policy), where the number of accommodation was the most important variable. 
...
We can differentiate the price prediction for the property type by comparing an apartment and a room. On the graph below, we can see that an apartment is predicted to have higher price than a room in average. It was similar in the London case study mention above, where entire home/apartment type of room has the highest predicted price. 
...
In the case study about Airbnb price prdiction in London, the differences between each subsamples were smaller. The RMSE/price at the large apartment was 0.43, at the small apartment was 0.46. As for the property type, the RMSE/price was 0.46 at the apartment type and 0.56 at the house type of property. 
...
In this project, we attempted to predict the price of an Airbnb in Lisbon using publicly available data. As our client required, we focused on Lisbon and excluded outer districts and the scope of the prediction was accommodations for 2-6 guests. We created five different models and compared them based on their accuracy. The random forest model turned to be the best model, as it has the lowest RMSE value. Based on our results, we recommend to use the random forest model for the price prediction, as it is proven to be more precise compared to the originally used OLS model. Thus, the revenue estimation could be more accurate for the future investment decisions. Even though the association between the price and the predictors are not clear, we can state the number of reviews, the number of accommodates and the amenities have an important role in the price prediction.
