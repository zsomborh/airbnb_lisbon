...
get_lns <- function(df, cont_vars, skewness_th = .5) {
    
    skews <- sapply(cont_vars,function(x) {
        skewness(df[,x])
    })
    
    names(skews) <- cont_vars
    
    for (i in seq_along(skews)){
        if(abs(skews[[i]]) > skewness_th){
            df[,paste0('ln_',names(skews)[i])] <- log(df[,names(skews)[i]])
        }
    }
